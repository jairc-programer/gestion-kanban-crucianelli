...
def obtener_siguiente_codigo_k(df):
    """
    Busca el número libre más pequeño (reciclado de borrados).
    Si no hay huecos, devuelve max + 1.
    """
    ...
    # Buscar el primer entero faltante comenzando desde 1
...
with tabs[0]:
    ...
    # Procesos adicionales para abastecimiento interno
    proceso_sel = None
    if es_interno:
        st.markdown("---")
    ...
    if st.button("💾 Guardar y Crear Kanban", type="primary"):
        ...
        if not material or not puesto_destino:
            ...
            # Control de duplicados (Material + Puesto Destino)
